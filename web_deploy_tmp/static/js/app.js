...
async function runAllBrowser(file) {
    ...
    // Step 2 — MAI (fast, await) + Azure (slow, background)
    updateProgressDots(2);
    showStatus('⚡ กำลังถอดความด้วย MAI...', 'info');
    ...
    // correctionDone prevents Azure from overwriting after AI finishes
    let azText = '', correctionDone = false;
    const azPromise = runAzureSDK(int16, STT_LANG, (txt) => {
        azText = txt;
        ...
        renderClickableText(outAz, txt);
        if (!correctionDone && txt && txt.length > (state.results.mai || '').length) {
            state.results.final = txt;
            renderClickableText(finalText, txt);
        }
    }).catch(err => {
        outAz.innerHTML = `<span class="text-red-400 text-xs">❌ ${err.message || 'ไม่สำเร็จ'}</span>`;
    });

    let maiText = '';
    try {
        maiText = await runMAIRest(file, STT_LANG);
        state.results.mai = maiText;
        renderClickableText(outMai, maiText);
    } catch (err) {
        outMai.innerHTML = `<span class="text-red-400 text-xs">❌ ${err.message || 'ไม่สำเร็จ'}</span>`;
    }

    // Show MAI result immediately (don't wait for Azure)
    updateProgressDots(3);
    const firstResult = maiText || azText;
    if (!firstResult) throw new Error('ถอดความไม่สำเร็จ (MAI)');
    ...
    let correctedText = firstResult;
    let aiError       = '';
    try {
        const [corrected, summary] = await Promise.all([
            runAISpeakerCorrection(firstResult).catch(e => {
                aiError = 'Correction: ' + e.message;
                console.error('[AI fail]', e);
                return firstResult;
            }),
            runAISummary(firstResult).catch(e => {
    ...
    if (aiError) {
        showStatus(`⚠️ AI Error: ${aiError}`, 'info');
    } else {
        showStatus('✅ เสร็จสิ้น! Azure ยังคงทำงานอยู่เบื้องหลัง...', 'ok');
        setTimeout(hideStatus, 5000);
    }

    // Azure continues in background (non-blocking)
    azPromise.catch(() => {});
}
